...
$interval = 10
$today=Get-Date
for ($days=1;$days -le $interval; $days++){

	$rundate = $(Get-Date).AddDays(-$days).ToString("yyyy-MM-dd") 
	#$rundate = $($today.adddays(-$days)).toshortdatestring() 
	...
	$dl_stat_file = "DL_stats_" + $outfile_date + ".csv" 
    ...
    # For Exchange 2013
	#$hts = get-exchangeserver |? {$_.serverrole -match "Mailbox"} |% {$_.name} 
